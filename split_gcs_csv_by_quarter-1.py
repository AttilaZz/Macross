...
def main():
    ...
    # trust_env=True : aiohttp respecte HTTPS_PROXY/HTTP_PROXY (indispensable derrière un proxy d'entreprise)
    ...
    # writers ouverts par fenêtre : {"2026-04-22_2026-07-21": writer membre CSV}
    ...
    def get_writer(period: str):
        if period not in writers:
            out_path = f"{output_prefix}/{period}.zip"
            print(f"Ouverture writer -> {out_path}")
            f = fs.open(out_path, "wb")
            gcs_files[period] = f
            zf = zipfile.ZipFile(f, "w", zipfile.ZIP_DEFLATED)
            zip_archives[period] = zf
            writers[period] = zf.open(f"{period}.csv", "w")
    ...
    # fermeture propre : membre CSV -> archive zip -> flush GCS (ordre important)
    for period in writers:
        writers[period].close()
        zip_archives[period].close()
        gcs_files[period].close()
        print(f"Finalisé : {output_prefix}/{period}.zip")
