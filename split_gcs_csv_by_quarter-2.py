...
def build_windows(start_date: pd.Timestamp, end_date: pd.Timestamp, months: int = 3) -> pd.DatetimeIndex:
    """Construit les bornes de fenêtres de `months` mois en remontant depuis end_date
...
def main():
    ...
    bounds = build_windows(start_date, end_date, args.window_months)
    labels = [
        f"{bounds[i].date()}_{(bounds[i + 1] - pd.Timedelta(days=1)).date()}"
        for i in range(len(bounds) - 1)
    ]
    print(f"Fenêtres de {args.window_months} mois (depuis l'ancre en remontant) :")
    for lb in labels:
        print(f"  - {lb}")
    ...
    def get_writer(period: str):
        if period not in writers:
            out_path = f"{output_prefix}/{period}.csv"
    ...
    for period in writers:
        ...
        print(f"Finalisé : {output_prefix}/{period}.csv")
