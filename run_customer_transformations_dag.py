...
poland_missing_in_rec = []
m7_missing_in_rec = []
m7_tag_if_rec = []
poland_tag_if_rec = []
if SNOWFLAKE_ENV == "REC":
    poland_missing_in_rec.append("source:customer_poland.interface__contract__poland")
    ...
    poland_tag_if_rec.append("tag:poland")

with DAG(
    dag_id="global_core_model_customer_dags_run_transformations_daily",
    schedule_interval=None,
    start_date=datetime(2024, 11, 21),
    catchup=False,
    max_active_runs=1,
    params=AIRFLOW_PARAMS,
    tags=["global-core-model-customer-dbt"],
    ...
    with TaskGroup("test_sources", tooltip="Run tests on sources") as section_test:
        ...
        test_sources_france = DbtTaskGroupSnowflake(
            group_id="test_sources_france",
            select=["source:customer_france"],
            exclude=["tag:safe"],
            test_behavior="after_each",
            **TASKS_COMMON_ARGUMENTS,
        )

        test_sources_bios = DbtTaskGroupSnowflake(
            group_id="test_sources_bios",
            select=["source:customer_bios"],
            exclude=["tag:safe"],
            test_behavior="after_each",
            **TASKS_COMMON_ARGUMENTS,
        )

        test_sources_poland = DbtTaskGroupSnowflake(
            group_id="test_sources_poland",
            select=["source:customer_poland"],
            exclude=["tag:safe"] + poland_missing_in_rec,
            test_behavior="after_each",
            **TASKS_COMMON_ARGUMENTS,
        )
        ...
        test_sources_m7 = DbtTaskGroupSnowflake(
            ...
            select=["source:customer_m7"],
            exclude=["tag:safe"] + m7_missing_in_rec,
            test_behavior="after_each",
            **TASKS_COMMON_ARGUMENTS,
        )

        test_sources_mauritius = DbtTaskGroupSnowflake(
            group_id="test_sources_mauritius",
            select=["source:customer_mauritius"],
            exclude=["tag:safe"],
            test_behavior="after_each",
            **TASKS_COMMON_ARGUMENTS,
        )

        test_sources_france >> test_sources_bios >> test_sources_poland >> test_sources_m7 >> test_sources_mauritius
    ...
    with TaskGroup("seeds", tooltip="Run seeds") as section_seeds:
        seeds_task = DbtTaskGroupSnowflake(
            group_id="customer_seeds",
            select=["tag:seeds,tag:customer"],
            exclude=["tag:safe"],
            **TASKS_COMMON_ARGUMENTS,
        )
    ...
    with TaskGroup("stagings_snapshots", tooltip="Run snapshots") as section_snapshots:
        ...
        run_customer_snapshots = DbtTaskGroupSnowflake(
            ...
            select=["resource_type:snapshot,tag:customer"],
        ...
        stagings_task = DbtTaskGroupSnowflake(
            group_id="stagings",
            select=["tag:staging,tag:customer"],
            exclude=["tag:safe"] + m7_tag_if_rec + poland_tag_if_rec,
            **TASKS_COMMON_ARGUMENTS,
        )

        run_contract_snapshots = DbtTaskGroupSnowflake(
            group_id="run_contract_snapshots",
            select=["resource_type:snapshot,tag:contract"],
            exclude=["tag:safe"] + m7_tag_if_rec + poland_tag_if_rec,
            **TASKS_COMMON_ARGUMENTS,
        )
        ...
        run_stagings_task_product = DbtTaskGroupSnowflake(
            ...
            select=["tag:staging,tag:product"],
            exclude=["tag:safe"] + m7_tag_if_rec + poland_tag_if_rec,
            **TASKS_COMMON_ARGUMENTS,
        )
        ...
        run_product_snapshots = DbtTaskGroupSnowflake(
            ...
            select=["resource_type:snapshot,tag:product"],
            exclude=["tag:safe"] + m7_tag_if_rec + poland_tag_if_rec,
            **TASKS_COMMON_ARGUMENTS,
        )
    ...
    with TaskGroup("data_marts", tooltip="Run dbt pipeline") as section_marts:
        marts_task = DbtTaskGroupSnowflake(
            ...
            exclude=["tag:poc_thoughtspot", "tag:safe"],
            project="global_core_model_customer",
            database_code="DWH",
            schema="GL",
            user_code=SNOWFLAKE_USER_CODE,
    ...
    section_test >> section_transco >> section_seeds >> section_snapshots >> section_marts >> section_alerting
